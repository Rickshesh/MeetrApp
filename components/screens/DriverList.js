  ...
  const getUsers = async () => {
    fetch(GET_USER_LIST) //S3 Link for Json
      ...
      .then((data) => {
        ...
        let topics = tempDevicesList.map(
          (element) => "aws/deviceUpdate/" + element
        );
        //setTopics(topics);
            ...
            {driverList.map((driver, index) => {
              return (
                <View key={index}>
                  ...
                  {Object.keys(showLocation).includes(driver.driverId) &&
                    showLocation[driver.driverId] == true && (
                      <Surface
                        elevation={2}
                        style={{ height: 200, margin: 10 }}
                      >
                        {mqttData[driver.deviceId] && (
                          <MapService
                            currentLocation={
                              mqttData[driver.deviceId].currentLocation
                            }
                            locationHistory={
                              mqttData[driver.deviceId].locationHistory
                            }
                            style={{ flex: 1 }}
                            icon="circle-slice-8"
                          />
                        ...
                        <IconButton
                          icon="chevron-up"
                          onPress={() =>
                            _setShowLocation(driver.driverId, false)
                          }
                          style={{
                            position: "absolute",
                            right: 2,
